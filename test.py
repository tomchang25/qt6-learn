...
class ProxyStyle(QtWidgets.QProxyStyle):
    def drawControl(self, element, opt, painter, widget):
        if element == QtWidgets.QStyle.ControlElement.CE_TabBarTabLabel:
            ic = self.pixelMetric(QtWidgets.QStyle.PixelMetric.PM_TabBarIconSize)
            r = QtCore.QRect(opt.rect)
            w = (
                0
                if opt.icon.isNull()
                else opt.rect.width() + self.pixelMetric(QtWidgets.QStyle.PixelMetric.PM_TabBarIconSize)
            )
            # r.setHeight(opt.fontMetrics.boundingRect().width() + w)
